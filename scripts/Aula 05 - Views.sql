...
--                                                                    QUERY PLAN                                                                   
-- ------------------------------------------------------------------------------------------------------------------------------------------------
--  Limit  (cost=131.82..131.83 rows=3 width=76) (actual time=36.067..36.071 rows=3 loops=1)
--    ->  Sort  (cost=131.82..134.02 rows=880 width=76) (actual time=36.066..36.069 rows=3 loops=1)
--          Sort Key: (sum(((op.quantity)::numeric * op.unit_price))) DESC
--          Sort Method: top-N heapsort  Memory: 25kB
--          ->  HashAggregate  (cost=109.44..120.44 rows=880 width=76) (actual time=36.046..36.053 rows=7 loops=1)
--                Group Key: p.id
--                Batches: 1  Memory Usage: 49kB
--                ->  Hash Join  (cost=61.75..92.52 rows=1354 width=56) (actual time=36.025..36.032 rows=12 loops=1)
--                      Hash Cond: (op.product_id = p.id)
--                      ->  Hash Join  (cost=31.95..59.14 rows=1354 width=24) (actual time=0.755..0.760 rows=12 loops=1)
--                            Hash Cond: (op.order_id = o.id)
--                            ->  Seq Scan on orders_products op  (cost=0.00..23.60 rows=1360 width=28) (actual time=0.497..0.498 rows=13 loops=1)
--                            ->  Hash  (cost=21.00..21.00 rows=876 width=4) (actual time=0.219..0.220 rows=9 loops=1)
--                                  Buckets: 1024  Batches: 1  Memory Usage: 9kB
--                                  ->  Seq Scan on orders o  (cost=0.00..21.00 rows=876 width=4) (actual time=0.211..0.213 rows=9 loops=1)
--                                        Filter: (status <> 'canceled'::text)
--                                        Rows Removed by Filter: 1
--                      ->  Hash  (cost=18.80..18.80 rows=880 width=36) (actual time=35.252..35.253 rows=9 loops=1)
--                            Buckets: 1024  Batches: 1  Memory Usage: 9kB
--                            ->  Seq Scan on products p  (cost=0.00..18.80 rows=880 width=36) (actual time=35.230..35.233 rows=9 loops=1)
--  Planning Time: 38.137 ms
--  Execution Time: 36.197 ms

--                                                        QUERY PLAN                                                        
-- -------------------------------------------------------------------------------------------------------------------------
--  Limit  (cost=27.88..27.89 rows=3 width=76) (actual time=1.275..1.276 rows=3 loops=1)
--    ->  Sort  (cost=27.88..29.83 rows=780 width=76) (actual time=1.274..1.275 rows=3 loops=1)
--          Sort Key: total_vendido DESC
--          Sort Method: top-N heapsort  Memory: 25kB
--          ->  Seq Scan on mv_top_products  (cost=0.00..17.80 rows=780 width=76) (actual time=1.211..1.213 rows=7 loops=1)
